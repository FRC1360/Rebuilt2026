...
public class TurretSubsystem extends SubsystemBase {
    ...
    public final Trigger turretAtPassingTarget;
    ...
    public Rotation2d getCurrentRobotRelativeRotation() {
        return Rotation2d.fromDegrees(this.getCurrentEncoderAngle())
                .plus(TurretConstants.ROBOT_TO_TURRET_CENTER.getRotation());
    }
    ...
    private double calculateWrapAround(Rotation2d target) {
        ...
        // Determines the co-terminal angles which are the negative and positive paths
        // that the turret will take.
        ...
        // If the turret is crossing the positive threshold from the positive direction,
        // take the negative path.
        if (currentAngle > 0 && negativePath > TurretConstants.NEGATIVE_THRESHOLD) {
            return negativePath;

            // If the turret is crossing the negative threshold from the negative direction,
            // take the positive path.
        } else if (currentAngle < 0 && positivePath < TurretConstants.POSITIVE_THRESHOLD) {
            return positivePath;
        }

        // Determines and returns the shorter path by magnitude with respect to the
        // current position.
