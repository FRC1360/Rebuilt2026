...
public class FieldZoneManager {
    ...
    // Each zone dimension holds a set of coordinates that goes from smaller to greater. They are also either x coordinates or y coordinates.
    
    //Alliance areas (only the y coordinates because the alliance zones cover the entire widths (x) of the field)
    ...
    //Middle zone (only the y coordinates in order to determine if the robot is in the middle before determining if it's in depot or human player)
    private double[] middleZoneDimensions = {Inches.of(182.11).in(Meters), Inches.of(469.11).in(Meters)};

    //Human player and depot zones (taken from the prespective of the blue alliance, only holds x coordinates)
    ...
    //Trench Y coordinates (the range of the trench's y positions, contains two possible ranges)
    private double[][] trenchesYPositions = {{Inches.of(158.61).in(Meters), Inches.of(205.61).in(Meters)}, {Inches.of(445.61).in(Meters), Inches.of(492.61).in(Meters)}};
     ...
     //Trench X coordinates (the range of the trench's x positions, contains two possible ranges)
    ...
    public boolean isRobotInDepot() {
        ...
        if (estimatedTurretPose.getY() > middleZoneDimensions[0] && estimatedTurretPose.getY() < middleZoneDimensions[1]){
            if (DriverStation.getAlliance().get() == Alliance.Blue){
                ...
                if (estimatedTurretPose.getX() > blueAllianceDepotZoneDimensions[0] - bufferZone / 2 && estimatedTurretPose.getX() < blueAllianceDepotZoneDimensions[1]){
                ...
                if (estimatedTurretPose.getX() > blueAllianceHumanPlayerZoneDimensions[0] && estimatedTurretPose.getX() < blueAllianceHumanPlayerZoneDimensions[1] + bufferZone / 2){
                    if (estimatedTurretPose.getX() > blueAllianceHumanPlayerZoneDimensions[0] && estimatedTurretPose.getX() < blueAllianceHumanPlayerZoneDimensions[1] - bufferZone / 2){
                        lastInDepot = true;
                        return true;
                    }
